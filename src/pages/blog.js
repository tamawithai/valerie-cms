        ...
        <section id="blog-list" className="py-20">
          <div className="container mx-auto px-4">
            <div className="text-center mb-16">
              <h1 className="text-4xl md:text-5xl font-bold mb-4">Blog Valerie</h1>
              ...
              {articles.length > 0 ? (
                articles.map((article, index) => (
                  <Link key={article.id} href={`/blog/${article.id}`} legacyBehavior>
                    <a className="block glass-card p-4 overflow-hidden glass-card-hover" style={{animationDelay: `${index * 100}ms`}}>
                      <div className="relative mb-4">
                        {/* Ganti dengan gambar dari database jika ada, jika tidak, gunakan placeholder */}
                        <img src={article.imageUrl || 'https://images.unsplash.com/photo-1556761175-5973dc0f32e7?auto=format&fit=crop&w=800&q=60'} alt={article.title} className="w-full h-48 object-cover rounded-lg" />
                      </div>
                      <div className="p-2">
                        ...
                        <div className="border-t border-white/10 pt-3">
                          <p className="text-gray-400 text-xs">
                            {new Date(article.createdAt).toLocaleDateString()} • {article.author.name}
                          </p>
                        </div>
                      </div>
                    </a>
                ...
                <p className="text-center col-span-full text-gray-400">Belum ada artikel yang dipublikasikan.</p>
              )}
            </div>
          </div>
        </section>
